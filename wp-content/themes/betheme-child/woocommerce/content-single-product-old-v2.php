				    ...
				    foreach($terms_slug as $term){
				        ...
				        foreach ($product->children as $moduloid) {
					        	...
					        	if($variacionobj->get_price()){
									// cantidad productos adicionales sf

									//$json = file_get_contents("https://tuttopelle.mx/wp-json/cantidad-productos-agrupados/v2/data");
									//$datacantidad = json_decode($json, true);
									//$childrenx = $product->get_children();

									//echo $variacionobj->get_price() . "<br>";
									...
									if(in_array($moduloid, $hijos_tb)){
										...
										//array_push($precios, $leproduct->get_price()*$productos_agrupados[$indice][2]);
									}
									else{
										//array_push($precios, $leproduct->get_price());
									...
									/*
									foreach ($sumarcantidad as $sumarow) {
										if($moduloid == $sumarow[1]){
											if ($variacionobj->get_sale_price()){
												$precio += $variacionobj->get_sale_price()*$sumarow[2];
												$precio_completo += $variacionobj->get_regular_price()*$sumarow[2];
											}else{
												$precio += $variacionobj->get_price()*$sumarow[2];
											}
										}
										else{
											if ($variacionobj->get_sale_price()){
								...
								if ( $product->get_id() == $datarow[0] && in_array($datarow[1], $childrenx) ) {
									...
									foreach ($childrenx as $childy) {
										...
										if($childyz->get_id() == $datarow[1]){
											if( !in_array($childyz->get_id(), $ids_agrupados)  ){
												echo "agrupado - " . $childyz->get_price()*$datarow[2] . "<br>";
												array_push($precios, $childyz->get_price()*$datarow[2]);
												array_push($ids_agrupados, $childyz->get_id());
											}
										}
										else{
											if( !in_array($childyz->get_id(), $ids) && !in_array($childyz->get_id(), $ids_agrupados)  ){
												echo "simple - " . $childyz->get_price()  . "<br>";
												
												array_push($precios, $childyz->get_price());
												array_push($ids, $childyz->get_id());
											}											
										}
									}
								}
