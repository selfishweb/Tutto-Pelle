	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				// SF - Get attribute term image
				$term_img = z_taxonomy_image_url($term->term_id);

				if ($term_img){
					$term_html = '<a rel="nofollow" class="sf-'. $term->term_id .'" href="' . esc_url( $link ) . '"><img style="width: 15px; height: 50px; border-radius: 50%;" src="'. esc_html( $term_img ) . '">' . esc_html( $term->name ) . '</a>';
				}else{
					$term_html = '<a rel="nofollow" class="sf-'. $thumb_id .'" href="' . esc_url( $link ) . '">' . esc_html( $term->name ) . '</a>';
				}
			} else {
				$link      = false;
