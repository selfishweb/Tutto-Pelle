	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				...
				// SF - Obtenemos imagen del color
				$imgurl = z_taxonomy_image_url($term->term_id);
				
				$term_html = '<a style="display: flex;" rel="nofollow" href="' . esc_url( $link ) . '"><img src="'. $imgurl .'" style="width: 15px; height: 15px; border-radius: 50%; align-self: center;"><span>' . esc_html( $term->name ) . '</span></a>';
			} else {
				$link      = false;
			...
			echo '<li class="sf woocommerce-widget-layered-nav-list__item wc-layered-nav-term ' . ( $option_is_set ? 'woocommerce-widget-layered-nav-list__item--chosen chosen' : '' ) . '">';
